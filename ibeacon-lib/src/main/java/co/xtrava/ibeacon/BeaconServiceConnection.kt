        ...
        override fun handleMessage(msg: Message) {
            ...
            when (msg.what) {
                ServiceProxy.MSG_REGION_ENTERED -> {
                    ...
                    region = mRegionsByHash.get(hash, BeaconRegion(uuid, major, minor))
        ...
        override fun handleMessage(msg: Message) {
            when (msg.what) {
                ServiceProxy.MSG_BEACONS_IN_REGION -> {
                    ...
                    val region = mRegionsByHash.get(hash, BeaconRegion(regionUuid, regionMajor, regionMinor))

                    mRegionsByHash.put(hash, region)
                    val beaconsCache = mBeaconsByAddressHash
                    val args = msg.data
                    val beaconsCount = args.getInt(ServiceProxy.EXTRA_COUNT)
                    val beacons: MutableList<Beacon> = ArrayList(beaconsCount)

                    if (beaconsCount > 0) {
                        val addresses = args.getStringArray(ServiceProxy.EXTRA_ADDRESSES) as Array<String>
                        val uuids = args.getParcelableArray(ServiceProxy.EXTRA_UUIDS) as Array<Parcelable>
                        val numbers = args.getIntArray(ServiceProxy.EXTRA_NUMBERS) as IntArray
                        val accuracies = args.getFloatArray(ServiceProxy.EXTRA_ACCURACIES) as FloatArray
                        val rssis = args.getIntArray(ServiceProxy.EXTRA_RSSI_VALUES) as IntArray
